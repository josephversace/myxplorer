import type { FileMetaData, SystemTime } from '../../Typings/fileMetaData';
import type { DirectoryData } from '../types/directory';
import IIMApiClient from './apiClient';
import type { AuthResult, BulkOperation, BulkOperationResult, SearchQuery, SearchResult, VirtualFile, Workspace } from './types';
...
export class VirtualWorkspaceService {
	...
	private async getWorkspacesListView(): Promise<DirectoryData> {
		console.log('Getting workspaces list view...');

		try {
			const workspaces = await this.apiClient.getWorkspacesAsync();
			console.log('Raw workspaces:', workspaces);

			const files: FileMetaData[] = workspaces.map((workspace) => ({
				file_path: `workspace://${workspace.id}`,
				...
				is_file: false,
				size: 0,
				last_modified: this.toSystemTime(workspace.updatedAt),
				created: this.toSystemTime(workspace.createdAt),
				last_accessed: this.toSystemTime(workspace.updatedAt), // Add this line
			}));

			console.log('Mapped workspace files:', files);
			...
			console.error('Error getting workspaces list:', error);
			throw error;
		}
	}

	async listDirectory(virtualPath: string): Promise<DirectoryData> {
		if (virtualPath === 'workspace://list' || virtualPath === 'workspace://' || virtualPath === 'workspaces') {
			return this.getWorkspacesListView();
		}
	...
	async search(virtualPath: string, pattern: string): Promise<FileMetaData[]> {
		...
		return result.files
			...
			.filter((file) =>
				info.relativePath === '/'
					? true
					: this.isDescendant(file.normalizedPath, info.relativePath) || file.normalizedPath === info.relativePath
			)
	...
	private ensureDate(dateInput: string | Date | undefined): Date {
		if (!dateInput) {
	...
	private async ensureWorkspaceCache(workspaceId: string): Promise<WorkspaceCache> {
		...
		const [workspace, files] = await Promise.all([this.apiClient.getWorkspaceAsync(workspaceId), this.apiClient.getFilesAsync(workspaceId)]);
	...
	private normalizeFile(file: VirtualFile): NormalizedVirtualFile {
		console.log('Normalizing file:', file.fileName);
		console.log('Original file dates:', {
		...
		try {
			...
			console.error('Error in normalizeFile:', error);
			throw error;
		}
	}
	...
	private toFileMeta(workspaceId: string, file: NormalizedVirtualFile): FileMetaData {
		// Always provide valid SystemTime objects, never undefined
		...
		return {
			...
			is_file: true,
			size: file.fileSize,
			// Always provide valid SystemTime objects
			last_modified: this.toSystemTime(file.updatedAt || file.createdAt || fallbackDate),
			created: this.toSystemTime(file.createdAt || fallbackDate),
			last_accessed: this.toSystemTime(file.updatedAt || file.createdAt || fallbackDate),
		};
	}
	...
	private toSystemTime(dateInput: string | Date | undefined): SystemTime {
		const date = dateInput ? this.ensureDate(dateInput) : new Date();
		return {
